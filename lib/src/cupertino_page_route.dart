...
class _CupertinoBackGestureDetectorState<T>
  ...
  bool _needUseWillPop() {
    final controllerValue = _backGestureController?.controller.value ?? 1;

    return controllerValue <= 0.5 && _willPopCallback != null;
  }
  ...
  Future<void> _handleDragEnd(DragEndDetails details) async {
    assert(mounted);
    // assert(_backGestureController != null);
    late final bool doNotPop;

    if (_needUseWillPop()) {
      doNotPop = await _willPopCallback!() == RoutePopDisposition.doNotPop;
    } else {
      doNotPop = true;
    }

    if (doNotPop) {
      return _handleDragCancel();
    } else {
  ...
  void _handleDragCancel() {
    ...
    _backGestureController?.dragEnd(0.0, forceDragBack: true);
    _backGestureController = null;
  }
...
class _CupertinoBackGestureController<T> {
  ...
  void dragEnd(double velocity, {bool forceDragBack = false}) {
    ...
    if (velocity.abs() >= _kMinFlingVelocity) {
      ...
      animateForward = controller.value > 0.5 || forceDragBack;
    }
